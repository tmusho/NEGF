...
real(kind=8), allocatable, dimension(:) :: DH, DHO
...
subroutine Eigen_Plot
   ...
   open(unit=13, file='Eig_p.dat', status='new', action='write', iostat=err)
   write(var,*) 'Eig_p.dat'
   call Errors_Fileopen(err,var)

   do r = 1, Nb
      write(13,'(I5,E15.6,E15.6,E15.6)') r, sqrt(DH(r)),DH(r),sqrt(DH(r))*hbar_eV
   end do
